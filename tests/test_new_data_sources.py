...
from agent_platform.agents.stock_recap.data.sources.sector import EastmoneyPush2BoardsSource
from agent_platform.agents.stock_recap.data.sources.sector_fund_flow import fetch_sector_fund_flow
from agent_platform.agents.stock_recap.data.sources.us_movers import _parse_lines
...
def test_sector_fund_flow_normalizes(monkeypatch):
    ...
    with patch("agent_platform.agents.stock_recap.data.sources.sector_fund_flow.push2_clist", side_effect=_fake):
...
def test_limit_up_pool_aggregates_themes_and_high_tier():
    ...
    with patch("agent_platform.agents.stock_recap.data.sources.limit_up_pool.push2ex_zt_pool", return_value=fake_pool):
...
def test_eastmoney_push2_boards_fallback_returns_industry_and_concept():
    ...
    with patch("agent_platform.agents.stock_recap.data.sources.sector.push2_clist", side_effect=_fake):
...
def test_individual_fund_flow_normalizes(monkeypatch):
    ...
    with patch("agent_platform.agents.stock_recap.data.sources.individual_fund_flow.push2_clist", side_effect=_fake):
